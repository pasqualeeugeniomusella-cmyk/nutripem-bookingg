            ...
            <FadeIn delay={0.15} className="hero-photo-wrap">
              <div className="hero-photo-frame">
                ...
                <div className="hero-photo-card top">
                  ...
                  <div className="hpc-lbl">{lang === "ar" ? "أنثروبومترية" : lang === "zh" ? "体型测量师" : lang === "ru" ? "Антропометрист" : lang === "fr" ? "Anthropométriste" : "Antropometrista"}</div>
                </div>
                <div className="hero-photo-card bottom">
                  <div className="hpc-num">5+</div>
                  <div className="hpc-lbl">{lang === "ar" ? "سنوات خبرة" : lang === "zh" ? "年经验" : lang === "ru" ? "Лет опыта" : lang === "en" ? "Years exp." : lang === "fr" ? "Ans d'exp." : lang === "es" ? "Años exp." : lang === "pt" ? "Anos exp." : "Anni nel settore"}</div>
                </div>
              </div>
            </FadeIn>
          </div>
          ...
          <div className="wrap">
            ...
            <div className="profilo-grid">
              ...
              <FadeIn delay={0.2} className="profilo-right">
                ...
                <div className="profilo-numeri">
                  <div className="pn-card"><div className="pn-num">2</div><div className="pn-lbl">{lang==="en"?"Federico II Degrees" : lang === "fr" ? "Diplômes Federico II" : lang==="es"?"Títulos Federico II":lang==="pt"?"Diplomas Federico II":lang==="ar"?"درجات فيدريكو II":lang==="ru"?"Дипломов":lang==="zh"?"学位":"Lauree Federico II"}</div></div>
                  <div className="pn-card"><div className="pn-num">1</div><div className="pn-lbl">{lang==="en"?"Real Madrid Master" : lang === "fr" ? "Master Real Madrid" : lang==="es"?"Máster Real Madrid":lang==="pt"?"Master Real Madrid":lang==="ar"?"ماستر ريال مدريد":lang==="ru"?"Мастер Реал Мадрид":lang==="zh"?"皇马硕士":"Master Real Madrid"}</div></div>
                  <div className="pn-card accent-card"><div className="pn-num">5+</div><div className="pn-lbl">{lang==="en"?"Intl. Certifications" : lang === "fr" ? "Cert. internationales" : lang==="es"?"Cert. internacionales":lang==="pt"?"Cert. internacionais":lang==="ar"?"شهادات دولية":lang==="ru"?"Сертификатов":lang==="zh"?"国际证书":"Cert. internazionali"}</div></div>
                  <div className="pn-card"><div className="pn-num">ISAK 2</div><div className="pn-lbl">{lang==="en"?"Anthropometrist" : lang === "fr" ? "Anthropométriste" : lang==="es"?"Antropometrista":lang==="pt"?"Antropometrista":lang==="ar"?"أنثروبومترية":lang==="ru"?"Антропометрист":lang==="zh"?"体型测量师":"Antropometrista"}</div></div>
                </div>
            ...
            <FadeIn delay={0.1}>
              <div className="milestones-label">
                <span className="milestone-kicker">📍 {lang==="en"?"Key milestones" : lang === "fr" ? "Étapes clés" : lang==="es"?"Hitos clave":lang==="pt"?"Marcos principais":lang==="ar"?"المحطات الرئيسية":lang==="ru"?"Ключевые этапы":lang==="zh"?"关键里程碑":"Tappe fondamentali"}</span>
              </div>
            </FadeIn>
            <div className="milestones-row">
              {[
                { year:"2020", icon:"🎓", label:lang==="en"?"MSc Nutrition" : lang === "fr" ? "Master Nutrition" : lang==="es"?"Máster Nutrición":lang==="pt"?"Mestrado Nutrição":lang==="ar"?"ماجستير تغذية":lang==="ru"?"Магистр питания":lang==="zh"?"营养学硕士":"Laurea Magistrale Nutrizione", sub:"Federico II" },
                { year:"2021", icon:"🔬", label:lang==="en"?"FNOB Biologist" : lang === "fr" ? "Biologiste FNOB" : lang==="es"?"Biólogo FNOB":lang==="pt"?"Biólogo FNOB":lang==="ar"?"بيولوجي FNOB":lang==="ru"?"Биолог FNOB":lang==="zh"?"生物学家FNOB":"Biologo FNOB", sub:"AA_087812" },
                { year:"2022–23", icon:"🏟️", label:lang==="en"?"Real Madrid CF" : lang === "fr" ? "Real Madrid CF" : lang==="es"?"Real Madrid CF":lang==="pt"?"Real Madrid CF":lang==="ar"?"ريال مدريد CF":lang==="ru"?"Реал Мадрид CF":lang==="zh"?"皇家马德里CF":"Tirocinio Real Madrid", sub:lang==="en"?"Internship + Master" : lang === "fr" ? "Stage + Master" : lang==="es"?"Prácticas + Máster":lang==="pt"?"Estágio + Master":lang==="ar"?"تدريب + ماستر":lang==="ru"?"Стажировка + Мастер":lang==="zh"?"实习+硕士":"Tirocinio + Master" },
                { year:"2023", icon:"📏", label:"ISAK Level 2", sub:lang==="en"?"Anthropometrist" : lang === "fr" ? "Anthropométriste" : lang==="es"?"Antropometrista":lang==="pt"?"Antropometrista":lang==="ar"?"أنثروبومترية":lang==="ru"?"Антропометрист":lang==="zh"?"体型测量师":"Antropometrista" },
                { year:"2023–26", icon:"🏀", label:lang==="en"?"PSA Basket" : lang === "fr" ? "PSA Basket" : lang==="es"?"PSA Basket":lang==="pt"?"PSA Basket":lang==="ar"?"PSA باسكت":lang==="ru"?"PSA Basket":lang==="zh"?"PSA篮球":"PSA Basket", sub:lang==="en"?"Sports Nutritionist" : lang === "fr" ? "Nutritionniste du Sport" : lang==="es"?"Nutricionista deportivo":lang==="pt"?"Nutricionista esportivo":lang==="ar"?"أخصائي تغذية":lang==="ru"?"Нутрициолог":lang==="zh"?"运动营养师":"Nutrizionista Sportivo" },
                { year:"2026", icon:"⚽", label:lang==="en"?"FIFA + FIGC + Barça" : lang === "fr" ? "FIFA + FIGC + Barça" : lang==="ar"?"فيفا + FIGC + برشلونة":lang==="zh"?"FIFA+FIGC+巴萨":"FIFA + FIGC + Barça", sub:lang==="en"?"3 top certifications" : lang === "fr" ? "3 certifications majeures" : lang==="es"?"3 top certificaciones":lang==="pt"?"3 top certificações":lang==="ar"?"3 شهادات قمة":lang==="ru"?"3 топ сертификата":lang==="zh"?"3项顶级证书":"3 certificazioni top" },
              ].map((m, i) => (
            ...
            <div className="exp-highlight-row">
              {[
                { icon:"🏟️", color:"#fff3e0", border:"#f5c87a", title:lang==="en"?"Real Madrid CF — Internship" : lang === "fr" ? "Real Madrid CF — Stage" : lang==="es"?"Real Madrid CF — Prácticas":lang==="pt"?"Real Madrid CF — Estágio":lang==="ar"?"ريال مدريد CF — تدريب":lang==="ru"?"Реал Мадрид CF — Стажировка":lang==="zh"?"皇马CF-实习":"Real Madrid CF — Tirocinio", period:"Dic 2022 – Giu 2023", desc:lang==="en"?"Nutrition department of Real Madrid's Medical Services, working with elite athletes." : lang === "fr" ? "Département nutrition des Services Médicaux du Real Madrid, avec des athlètes d'élite." : lang==="es"?"Área de nutrición de los Servicios Médicos del Real Madrid, con atletas de élite.":lang==="pt"?"Área de nutrição dos Serviços Médicos do Real Madrid, com atletas de elite.":lang==="ar"?"قسم التغذية في الخدمات الطبية لريال مدريد مع الرياضيين من النخبة.":lang==="ru"?"Отдел питания медицинских служб Реал Мадрид, работа с элитными спортсменами.":lang==="zh"?"皇家马德里医疗部门营养科，与精英运动员合作。":"Area nutrizione Servizi Medici del Real Madrid, con atleti d'élite.", featured:true },
                { icon:"🏀", color:"#e8f0fb", border:"#a9c4ee", title:lang==="en"?"PSA Basket Sant'Antimo" : lang === "fr" ? "PSA Basket Sant'Antimo" : lang==="ar"?"PSA Basket Sant'Antimo":lang==="zh"?"PSA篮球":"PSA Basket Sant'Antimo", period:"Nov 2023 – Mag 2026", desc:lang==="en"?"Sports nutritionist for the club across National Series B, Series C and youth sector." : lang === "fr" ? "Nutritionniste du sport du club en Serie B Nationale, Serie C et secteur jeunes." : lang==="es"?"Nutricionista deportivo del club en Serie B Nacional, Serie C y cantera.":lang==="pt"?"Nutricionista esportivo do clube na Série B Nacional, Série C e setor juvenil.":lang==="ar"?"أخصائي تغذية رياضية للنادي في الدوري الوطني B والدوري C والقطاع الشبابي.":lang==="ru"?"Нутрициолог клуба в Национальной серии B, серии C и молодёжном секторе.":lang==="zh"?"俱乐部运动营养师，覆盖全国B联赛、C联赛及青训。":"Nutrizionista del club tra Serie B Nazionale, Serie C e settore giovanile.", featured:false },
                { icon:"🦇", color:"#fcedf0", border:"#eaaab6", title:lang==="en"?"Valencia CF Academy" : lang === "fr" ? "Valencia CF Academy" : lang==="ar"?"أكاديمية فالنسيا CF":lang==="zh"?"巴伦西亚CF学院":"Valencia CF Academy", period:"Nov 2024 – Lug 2025", desc:lang==="en"?"Applied nutrition for youth football — Technical Partnership Caserta, AG Soccer School." : lang === "fr" ? "Nutrition appliquée au football des jeunes — Technical Partnership Caserta, AG Soccer School." : lang==="es"?"Nutrición aplicada al fútbol juvenil — Technical Partnership Caserta, AG Soccer School.":lang==="pt"?"Nutrição aplicada ao futebol juvenil — Technical Partnership Caserta, AG Soccer School.":lang==="ar"?"تغذية مطبقة لكرة القدم الشبابية — Technical Partnership Caserta, AG Soccer School.":lang==="ru"?"Питание для молодёжного футбола — Technical Partnership Caserta, AG Soccer School.":lang==="zh"?"青训足球应用营养——卡塞尔塔技术合作，AG足球学校。":"Nutrizione applicata al calcio giovanile — Technical Partnership Caserta.", featured:false },
              ].map((e, i) => (
                <FadeIn key={i} delay={i * 0.1}>
                  <div className="exp-hl-card" style={{ background:e.color, borderColor:e.border }}>
                    {e.featured && <span className="exp-hl-feat">⭐ {lang==="en"?"Elite exp." : lang === "fr" ? "Exp. d'élite" : lang==="es"?"Exp. élite":lang==="pt"?"Exp. elite":lang==="ar"?"خبرة نخبة":lang==="ru"?"Элит. опыт":lang==="zh"?"精英经历":"Esperienza d'élite"}</span>}
